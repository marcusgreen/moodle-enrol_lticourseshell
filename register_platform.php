...
/**
 * lticourseshell 1.3 page to create or edit a platform registration.
 *
 * This page is only used by lticourseshell 1.3. Older versions do not require platforms to be registered with the tool during
 * registration.
 *
...
use core\output\notification;
use enrol_lticourseshell\local\ltiadvantage\form\create_registration_form;
use enrol_lticourseshell\local\ltiadvantage\form\platform_registration_form;
...
use enrol_lticourseshell\local\ltiadvantage\service\application_registration_service;
...
$toolregistrationurl = new moodle_url($CFG->wwwroot . "/" . $CFG->admin . "/settings.php",
    ['section' => 'enrolsettingslticourseshell_registrations']);
...
$pagesetup = function(string $pagetitle) {
    ...
    navigation_node::override_active_url(
        new moodle_url('/admin/settings.php', ['section' => 'enrolsettingslticourseshell_registrations'])
    );
    admin_externalpage_setup('enrolsettingslticourseshell_registrations_edit', '', null, '', ['pagelayout' => 'admin']);
...
if ($action == 'view') {
    ...
    global $PAGE;
    $pagesetup(get_string('registerplatformedit', 'enrol_lticourseshell'));
    $pageurl = new moodle_url('/enrol/lticourseshell/register_platform.php', ['action' => 'view', 'regid' => $regid]);

    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('registerplatformedit', 'enrol_lticourseshell'));

    $renderer = $PAGE->get_renderer('enrol_lticourseshell');
    echo $renderer->render_registration_view($regid, $tabselect);
    echo $OUTPUT->footer();
    die();

} else if ($action === 'add') {
    $pagesetup(get_string('registerplatformadd', 'enrol_lticourseshell'));
    $pageurl = new moodle_url('/enrol/lticourseshell/register_platform.php', ['action' => 'add']);
    ...
    if ($data = $mform->get_data()) {
        ...
        redirect(new moodle_url('/enrol/lticourseshell/register_platform.php',
            ['action' => 'view', 'regid' => $draft->get_id(), 'tabselect' => 'tooldetails']));

    } else if (!$mform->is_cancelled()) {
        // Display the first step of registration creation.
        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('registerplatformadd', 'enrol_lticourseshell'));
        $mform->display();
        echo $OUTPUT->footer();
        die();
    }
    ...
    $pageurl = new moodle_url('/enrol/lticourseshell/register_platform.php', ['action' => 'edit', 'regid' => $regid]);
    $viewurl = new moodle_url('/enrol/lticourseshell/register_platform.php', ['action' => 'view', 'regid' => $regid]);
    ...
    if (($data = $mform->get_data()) && confirm_sesskey()) {
        ...
        redirect($viewurl, get_string('registerplatformeditnotice', 'enrol_lticourseshell'), null,
    ...
    $regid = required_param('regid', PARAM_INT);
    $pagesetup(get_string('registerplatformdelete', 'enrol_lticourseshell'));

    if (!optional_param('confirm', false, PARAM_BOOL)) {
        $continueparams = ['action' => 'delete', 'regid' => $regid, 'sesskey' => sesskey(), 'confirm' => true];
        $continueurl = new moodle_url('/enrol/lticourseshell/register_platform.php', $continueparams);
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->confirm(
            get_string('registerplatformdeleteconfirm', 'enrol_lticourseshell', format_string($appreg->get_name())),
            $continueurl,
            $toolregistrationurl
        );
        echo $OUTPUT->footer();
        ...
        redirect($toolregistrationurl,
            get_string('registerplatformdeletenotice', 'enrol_lticourseshell'), null,  notification::NOTIFY_SUCCESS);
    }
}
