 ...
 */
function enrol_lti_cs_verify_private_key() {

    $name = 'lticourseshell_13_kid';
    $key = get_config('enrol_lticourseshell', $name);

    // If we already generated a valid key, no need to check.
    if (empty($key)) {
        // Create the private key.
        $kid = bin2hex(openssl_random_pseudo_bytes(10));
        set_config($name, $kid, 'enrol_lticourseshell');
        $config = array(
            "digest_alg" => "sha256",
        ...
        if (!empty($privatekey)) {
            set_config('lticourseshell_13_privatekey', $privatekey, 'enrol_lticourseshell');
        } else {
            return get_string('opensslconfiginvalid', 'enrol_lticourseshell');
        }
    }
