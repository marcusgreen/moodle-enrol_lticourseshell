...
final class lib_test extends \lti_advantage_testcase {
    ...
    /**
     * Test for enrol_lti_cs_plugin::delete_instance().
     */
    public function test_delete_instance(): void {
        ...
        $lticourseshelluser = User::fromResourceLink($resourcelink, '');
        $lticourseshelluser->lticourseshellResultSourcedId = 'testlticourseshellResultSourcedId';
        ...
        $this->assertTrue($DB->record_exists('enrol_lti_cs_tool_consumer_map', $mappingparams));
        $this->assertTrue($DB->record_exists('enrol_lti_cs_lticourseshell2_consumer', [ 'id' => $consumer->getRecordId() ]));
        $this->assertTrue($DB->record_exists('enrol_lti_cs_lticourseshell2_resource_link', [ 'id' => $resourcelink->getRecordId() ]));
        $this->assertTrue($DB->record_exists('enrol_lti_cs_lticourseshell2_user_result', [ 'id' => $lticourseshelluser->getRecordId() ]));

        // Perform deletion.
        $enrollticourseshell = new enrol_lti_cs_plugin();
        $instance = $DB->get_record('enrol', ['id' => $tool->enrolid]);
        $enrollticourseshell->delete_instance($instance);

        // Check that the related records have been deleted.
        $this->assertFalse($DB->record_exists('enrol_lti_cs_tool_consumer_map', $mappingparams));
        $this->assertFalse($DB->record_exists('enrol_lti_cs_lticourseshell2_consumer', [ 'id' => $consumer->getRecordId() ]));
        $this->assertFalse($DB->record_exists('enrol_lti_cs_lticourseshell2_resource_link', [ 'id' => $resourcelink->getRecordId() ]));
        $this->assertFalse($DB->record_exists('enrol_lti_cs_lticourseshell2_user_result', [ 'id' => $lticourseshelluser->getRecordId() ]));

        // Check that the enrolled users and the tool instance has been deleted.
        $this->assertFalse($DB->record_exists('enrol_lti_cs_users', [ 'toolid' => $tool->id ]));
        $this->assertFalse($DB->record_exists('enrol_lti_cs_tools', [ 'id' => $tool->id ]));
     ...
     *
     * @covers \enrol_lti_cs_plugin::delete_instance
     */
    public function test_delete_instance_lticourseshell_advantage(): void {
        global $DB;
        // Setup.
        [
            $course,
        ...
        // Verify data exists.
        $this->assertEquals(1, $DB->count_records('enrol_lti_cs_user_resource_link'));
        $this->assertEquals(1, $DB->count_records('enrol_lti_cs_resource_link'));
        $this->assertEquals(1, $DB->count_records('enrol_lti_cs_app_registration'));
        $this->assertEquals(1, $DB->count_records('enrol_lti_cs_deployment'));
        $this->assertEquals(1, $DB->count_records('enrol_lti_cs_context'));
        $this->assertEquals(1, $DB->count_records('enrol_lti_cs_users'));
        ...
        $this->assertEquals(0, $DB->count_records('enrol_lti_cs_user_resource_link'));
        $this->assertEquals(0, $DB->count_records('enrol_lti_cs_resource_link'));
        $this->assertEquals(0, $DB->count_records('enrol_lti_cs_users'));
    ...
    public function test_get_user_enrolment_actions(): void {
        ...
        $pluginname = 'lticourseshell';

        // Only enable the lticourseshell enrol plugin.
        ...
        // Teachers don't have enrol/lticourseshell:unenrol capability by default. Login as admin for simplicity.
        $this->setAdminUser();
        ...
        // lticourseshell enrolment has 1 enrol actions for active users -- unenrol.
     ...
     *
     * @covers \enrol_lti_cs_pre_course_module_delete
     */
