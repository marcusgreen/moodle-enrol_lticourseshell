...
use enrol_lticourseshell\task\sync_members;
...
final class sync_members_test extends \advanced_testcase {
    ...
    public function setUp(): void {
        ...
        $this->tool = helper::get_lticourseshell_tool($tool->id);
        ...
        for ($i = 1; $i <= $count; $i++) {
            ...
            $user->lticourseshellUserId = 'user' . $i;
        ...
        $this->context = Context::fromConsumer($this->consumer, 'testlticourseshellcontextid');
        $this->context->save();
    ...
    /**
     * Test for sync_members::execute() when enrol_lticourseshell is disabled.
     */
    public function test_execute_enroldisabled(): void {
        ...
        $this->task->execute();
        $output = ob_get_clean();
        $message = 'Skipping task - ' . get_string('enrolisdisabled', 'enrol_lticourseshell');
        $this->assertStringContainsString($message, $output);
    }
    ...
    public function test_get_name(): void {
        $this->assertEquals(get_string('tasksyncmembers', 'enrol_lticourseshell'), $this->task->get_name());
    }
    ...
    protected function enable_auth() {
        ...
        if (!in_array('lticourseshell', $auths)) {
            $auths[] = 'lticourseshell';
        }
