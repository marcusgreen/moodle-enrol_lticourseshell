...
namespace enrol_lticourseshell\local\ltiadvantage\task;

use enrol_lticourseshell\helper;
use enrol_lticourseshell\local\ltiadvantage\entity\user;
...
require_once(__DIR__ . '/../lticourseshell_advantage_testcase.php');

/**
 * Tests for the enrol_lticourseshell\local\ltiadvantage\task\sync_members scheduled task.
 *
 * @package enrol_lticourseshell
 * @copyright 2021 Jake Dallimore <jrhdallimore@gmail.com>
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @coversDefaultClass \enrol_lticourseshell\local\ltiadvantage\task\sync_members
 */
final class sync_members_test extends \lticourseshell_advantage_testcase {
     ...
     *
     * @param array $userids the array of lticourseshell user ids to use.
     * @param array|null $legacyuserids legacy user ids for the lticourseshell11_legacy_user_id property, null if not desired.
    ...
    protected static function get_mock_members_with_ids(
        ...
        foreach ($userids as $userid) {
            ...
            if ($legacyuserids) {
                $user['lticourseshell11_legacy_user_id'] = array_shift($legacyuserids);
            }
            if ($linklevel) {
                // Link-level memberships also include a message property.
                $user['message'] = [
                    'https://purl.imsglobal.org/spec/lti/claim/message_type' => 'lticourseshellResourceLinkRequest'
                ];
            }
            $users[] = $user;
        }
    ...
    /**
     * Check that all the given lticourseshellusers are enrolled in the course.
     *
     * @param \stdClass $course the course instance.
     * @param user[] $lticourseshellusers array of lticourseshell user instances.
     */
    protected function verify_course_enrolments(\stdClass $course, array $lticourseshellusers) {
        global $CFG;
        require_once($CFG->libdir . '/enrollib.php');
        $enrolledusers = get_enrolled_users(\context_course::instance($course->id));
        $this->assertCount(count($lticourseshellusers), $enrolledusers);
        $enrolleduserids = array_map(function($stringid) {
            return (int) $stringid;
        }, array_column($enrolledusers, 'id'));
        foreach ($lticourseshellusers as $lticourseshelluser) {
            $this->assertContains($lticourseshelluser->get_localid(), $enrolleduserids);
        }
    }
    ...
    public function test_get_name(): void {
        $this->assertEquals(get_string('tasksyncmembers', 'enrol_lticourseshell'), (new sync_members())->get_name());
    }
    ...
    public function test_resource_link_level_sync(): void {
        ...
        $instructoruser = $this->lticourseshell_advantage_user_authenticates('1');
        ...
        $lticourseshellusers = $userrepo->find_by_resource($resource->id);
        $this->assertCount(2, $lticourseshellusers);
        $this->verify_course_enrolments($course, $lticourseshellusers);
    }
     ...
     * @covers ::execute
     */
    public function test_resource_link_level_sync_multicourseshellple_resource_links(): void {
        $this->resetAfterTest();
        [$course, $resource] = $this->create_test_environment();
        ...
        $userrepo = new user_repository();
        $lticourseshellusers = $userrepo->find_by_resource($resource->id);
        $this->assertCount(10, $lticourseshellusers);
    ...
    public function test_user_profile_image_sync(): void {
        ...
        $userrepo = new user_repository();
        $lticourseshellusers = $userrepo->find_by_resource($resource->id);
        $this->assertCount(1, $lticourseshellusers);
        $this->verify_course_enrolments($course, $lticourseshellusers);

        // Verify user profile image has been updated.
        $this->verify_user_profile_image($lticourseshellusers[0]->get_localid());
    }
    ...
    public function test_context_level_sync(): void {
        ...
        $userrepo = new user_repository();
        $lticourseshellusers = $userrepo->find_by_resource($resource->id);
        $this->assertCount(3, $lticourseshellusers);
        $this->verify_course_enrolments($course, $lticourseshellusers);
    }
    ...
    public function test_sync_user_data(): void {
        ...
        $lticourseshellusers = $userrepo->find_by_resource($resource->id);
        $this->assertCount(5, $lticourseshellusers);
        $this->verify_course_enrolments($course, $lticourseshellusers);

        // Since user data wasn't included in the response, the users will have been synced using fallbacks,
        // so verify these.
        foreach ($lticourseshellusers as $lticourseshelluser) {
            $user = \core_user::get_user($lticourseshelluser->get_localid());
            // Firstname falls back to sourceid.
            $this->assertEquals($lticourseshelluser->get_sourceid(), $user->firstname);
            ...
            $issuersubhash = sha1($appreg->get_platformid() . '_' . $lticourseshelluser->get_sourceid());
            $this->assertEquals("enrol_lti_cs_13_{$issuersubhash}@example.com", $user->email);
        }
        ...
        foreach ($lticourseshellusers as $lticourseshelluser) {
            $user = \core_user::get_user($lticourseshelluser->get_localid());
            $mockmemberindex = array_search($lticourseshelluser->get_sourceid(), array_column($mockmembers, 'user_id'));
    ...
    public function test_membership_sync_disabled(): void {
        ...
        $lticourseshellusers = $userrepo->find_by_resource($resource->id);
        $this->assertCount(1, $lticourseshellusers);
        $this->assertEquals($mockuser['user_id'], $lticourseshellusers[0]->get_sourceid());
        $this->verify_course_enrolments($course, $lticourseshellusers);
    }
    ...
    public function test_sync_mode_enrol_and_unenrol(): void {
        ...
        $lticourseshellusers = $userrepo->find_by_resource($resource->id);
        $this->assertCount(4, $lticourseshellusers);
        $unenrolleduserids = ['2', '3'];
        $enrolleduserids = ['1', '4', '5', '6'];
        foreach ($lticourseshellusers as $lticourseshelluser) {
            $this->assertNotContains($lticourseshelluser->get_sourceid(), $unenrolleduserids);
            $this->assertContains($lticourseshelluser->get_sourceid(), $enrolleduserids);
        }
        $this->verify_course_enrolments($course, $lticourseshellusers);
    }
    ...
    public function test_sync_mode_unenrol_missing(): void {
        ...
        $lticourseshellusers = $userrepo->find_by_resource($resource->id);
        $this->assertCount(0, $lticourseshellusers);
    }
    ...
    public function test_sync_auth_disabled(): void {
        ...
        $this->expectOutputRegex("/Skipping task - Authentication plugin 'lticourseshell' is not enabled/");
        $this->assertCount(1, $userrepo->find_by_resource($resource->id));
    }

    /**
     * Test confirming that no sync takes place when the enrol_lticourseshell plugin is not enabled.
     *
     * @covers ::execute
     */
    public function test_sync_enrol_disabled(): void {
        ...
        // Verify that the sync didn't take place.
        $this->expectOutputRegex("/Skipping task - The 'Publish as lticourseshell tool' plugin is disabled/");
        $this->assertCount(1, $userrepo->find_by_resource($resource->id));
    }
    ...
    /**
     * Test confirming that preexisting, non-lticourseshell user accounts do not have their profiles or pictures updated during sync.
     *
     * @covers ::execute
     */
    public function test_sync_non_lticourseshell_linked_user(): void {
        $this->resetAfterTest();
        ...
        $authenticateduser = $this->lticourseshell_advantage_user_authenticates('123');
        ...
        // Run the member sync.
        $this->expectOutputRegex(
            "/Skipped profile sync for user '$authenticateduser->id'. The user does not belong to the lticourseshell auth method.\n" .
            "Skipped picture sync for user '$authenticateduser->id'. The user does not belong to the lticourseshell auth method/"
        );
        $task->execute();
     ...
     * @param array|null $resourceconfig array detailing config values to be used when creating the test enrol_lticourseshell instances.
    ...
    public function test_sync_enrolments_and_migration(?array $legacydata, ?array $resourceconfig, array $launchdata,
        ...
        // Perform the launch.
        $instructoruser = $this->lticourseshell_advantage_user_authenticates(
        ...
        // Verify enrolments.
        $lticourseshellusers = (new user_repository())->find_by_resource($resource->id);
        $enrolled = array_filter($expected['enrolments'], function($user) {
            return $user['is_enrolled'];
        });
        $this->assertCount(count($enrolled), $lticourseshellusers);
        $this->verify_course_enrolments($course, $lticourseshellusers);
        ...
        if ($legacydata) {
            ...
            foreach ($lticourseshellusers as $lticourseshelluser) {
                $this->assertArrayHasKey($lticourseshelluser->get_sourceid(), $expected['enrolments']);
                if (!$expected['enrolments'][$lticourseshelluser->get_sourceid()]['is_migrated']) {
                    // Those members who hadn't launched over 1p1 prior will have new lticourseshell user records created.
                    $this->assertNotContains((string)$lticourseshelluser->get_localid(), $legacyuserids);
                } else {
                    ...
                    $this->assertContains((string)$lticourseshelluser->get_localid(), $legacyuserids);
                }
            }
        }
    }
    ...
    public static function member_sync_data_provider(): array {
        ...
        return [
            ...
            'Default provisioning modes, mixed bag of users and roles' => [
                'legacy_data' => null,
                'resource_config' => [
                    'provisioningmodelearner' => \auth_plugin_lticourseshell::PROVISIONING_MODE_AUTO_ONLY,
                    'provisioningmodeinstructor' => \auth_plugin_lticourseshell::PROVISIONING_MODE_PROMPT_NEW_EXISTING
                ],
                'launch_data' => [
                    'user' => self::get_mock_launch_users_with_ids(['1p3_1'])[0],
                    'launch_migration_claim' => null,
            ...
            'All automatic provisioning, mixed bag of users and roles' => [
                ...
                'resource_config' => [
                    'provisioningmodelearner' => \auth_plugin_lticourseshell::PROVISIONING_MODE_AUTO_ONLY,
                    'provisioningmodeinstructor' => \auth_plugin_lticourseshell::PROVISIONING_MODE_AUTO_ONLY
                ],
                'launch_data' => [
                    'user' => self::get_mock_launch_users_with_ids(['1p3_1'])[0],
                    'launch_migration_claim' => null,
            ...
            'All prompt provisioning, mixed bag of users and roles' => [
                'legacy_data' => null,
                'resource_config' => [
                    'provisioningmodelearner' => \auth_plugin_lticourseshell::PROVISIONING_MODE_PROMPT_NEW_EXISTING,
                    'provisioningmodeinstructor' => \auth_plugin_lticourseshell::PROVISIONING_MODE_PROMPT_NEW_EXISTING
                ],
                'launch_data' => [
                    'user' => self::get_mock_launch_users_with_ids(['1p3_1'])[0],
