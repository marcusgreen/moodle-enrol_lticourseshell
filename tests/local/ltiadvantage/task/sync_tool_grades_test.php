...
/**
 * Tests for the enrol_lticourseshell\local\ltiadvantage\task\sync_tool_grades adhoc task.
 *
 * @package enrol_lticourseshell
 * @copyright 2023 David Pesce <david.pesce@exputo.com>
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @coversDefaultClass \enrol_lticourseshell\local\ltiadvantage\task\sync_tool_grades
 */
final class sync_tool_grades_test extends \lticourseshell_advantage_testcase {
    /**
    ...
    protected function get_task_with_mocked_grade_service($statuscode = '200', $mockexception = false): sync_tool_grades {
        $mockgradeservice = $this->getMockBuilder(lticourseshellAssignmentsGradesService::class)
     ...
     * @covers ::execute
     */
    public function test_grade_sync_multicourseshellple_resource_links(): void {
        $this->resetAfterTest();
    ...
    public function test_sync_grades_coupled_lineitem(): void {
        ...
        $agsclaim = [
            "scope" => ["https://purl.imsglobal.org/spec/lticourseshell-ags/scope/score"],
        ...
        $mockgradeservice = $this->createMock(lticourseshellAssignmentsGradesService::class);
        ...
        $mockgradeservice->expects($this->once())
            ->method('putGrade')
            ->with($this->isInstanceOf(lticourseshellGrade::class));
        $mocktask = $this->getMockBuilder(sync_tool_grades::class)
            ->onlyMethods(['get_ags'])
            ->getMock();
        $mocktask->method('get_ags')->willReturn($mockgradeservice);
    ...
    public function test_sync_grades_none_or_many_lineitems_activity_context(): void {
        ...
        $mockgradeservice->expects($this->once())
            ->method('putGrade')
            ->with($this->isInstanceOf(lticourseshellGrade::class), $this->isInstanceOf(lticourseshellLineitem::class));
        $mocktask = $this->getMockBuilder(sync_tool_grades::class)
            ->onlyMethods(['get_ags'])
            ->getMock();
        $mocktask->method('get_ags')->willReturn($mockgradeservice);
