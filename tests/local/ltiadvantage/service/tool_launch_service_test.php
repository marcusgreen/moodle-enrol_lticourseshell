...
namespace enrol_lticourseshell\local\ltiadvantage\service;
...
use enrol_lticourseshell\local\ltiadvantage\entity\context;
use enrol_lticourseshell\local\ltiadvantage\repository\application_registration_repository;
use enrol_lticourseshell\local\ltiadvantage\repository\context_repository;
 ...
 * @coversDefaultClass \enrol_lticourseshell\local\ltiadvantage\service\tool_launch_service
 */
final class tool_launch_service_test extends \lticourseshell_advantage_testcase {
    ...
    public function test_user_launches_tool(?array $legacydata, ?array $launchdata, array $expected): void {
        ...
        // As part of the launch, we expect to now have an lticourseshell-enrolled user who is recorded against the deployment.
    ...
    public function test_user_launches_tool_missing_custom_id(): void {
        ...
        $this->expectExceptionMessage(get_string('lticourseshelladvlauncherror:missingid', 'enrol_lticourseshell'));
        [$userid, $resource] = $launchservice->user_launches_tool($instructoruser, $mocklaunch);
    }
    ...
    public function test_user_launches_tool_invalid_custom_id(): void {
        ...
        $this->expectException(\moodle_exception::class);
        $this->expectExceptionMessage(get_string('lticourseshelladvlauncherror:invalidid', 'enrol_lticourseshell', 999999));
        [$userid, $resource] = $launchservice->user_launches_tool($instructoruser, $mocklaunch);
    }
    ...
    public function test_user_launches_tool_missing_registration(): void {
        ...
        $this->expectException(\moodle_exception::class);
        $this->expectExceptionMessage(get_string('lticourseshelladvlauncherror:invalidregistration', 'enrol_lticourseshell',
    ...
    public function test_user_launches_tool_missing_deployment(): void {
        ...
        $this->expectException(\moodle_exception::class);
        $this->expectExceptionMessage(get_string('lticourseshelladvlauncherror:invaliddeployment', 'enrol_lticourseshell',
    ...
    public function test_user_launches_tool_user_fields_updated(): void {
        ...
        global $DB;
        $DB->update_record('enrol_lti_cs_tools', $modresource);
    ...
    public function test_user_launches_tool_max_enrolment_start_restriction(): void {
        ...
        [$course, $modresource] = $this->create_test_environment(true, true, false,
            \enrol_lticourseshell\helper::MEMBER_SYNC_ENROL_NEW, false, false, time() + DAYSECS);
        $instructoruser = $this->getDataGenerator()->create_user();
        $mockinstructoruser = self::get_mock_launch_users_with_ids(['1'])[0];
        $mockinstructorlaunch = $this->get_mock_launch($modresource, $mockinstructoruser);
        $launchservice = $this->get_tool_launch_service();
    ...
    public static function aud_data_provider(): array {
        return [
            'valid, array having multicourseshellple entries with the first one being clientid' => [
            ...
            'invalid, array having multicourseshellple values where the first item is not clientid' => [
                'aud' => ['cat', 'dog', '123'],
                'expected' => [
                    'valid' => false,
                    'exception' => \moodle_exception::class,
                    'exceptionmessage' => get_string('lticourseshelladvlauncherror:invalidregistration', 'enrol_lticourseshell')
                ]
            ],
    ...
    /**
     * Test verifying how changes to lticourseshell-ags claim information is handled across different launches.
     *
     * @param array $agsclaim1 the lticourseshell-ags claim data to use in the first launch.
     * @param array $agsclaim2 the lticourseshell-ags claim data to use in the second launch.
    ...
    public function test_user_launches_tool_ags_claim_handling(array $agsclaim1, array $agsclaim2, array $expected): void {
        ...
        $lticourseshelluser = $userrepo->find_single_user_by_resource($userid, $resource->id);
        $resourcelink = $resourcelinkrepo->find_by_resource_and_user($resource->id, $lticourseshelluser->get_id())[0];
        ...
        // Launch again, with a new lticourseshell-ags claim.
    ...
    /**
     * Data provider for testing user_launches tool with varying mocked lticourseshell-ags claim data over several launches.
     *
