...
abstract class lticourseshell_advantage_testcase extends \advanced_testcase {
    ...
    /**
     * Helper to return a user which has been bound to the lticourseshell credentials provided and is deemed a valid linked user.
     *
     ...
     */
    protected function lticourseshell_advantage_user_authenticates(string $sub, array $migrationclaiminfo = []): \stdClass {
     ...
     * @param string $role the lticourseshell role to include in the user data.
    ...
    /**
     * Get a mock lticourseshellMessageLaunch object, as if a user had launched from a resource link in the platform.
     *
     ...
     * @param array|null $ags array representing the lticourseshell-ags claim info. Pass null to omit, empty array to use a default.
     ...
     * @return lticourseshellMessageLaunch the mock launch object with test launch data.
     */
    protected function get_mock_launch(\stdClass $resource, array $mockuser,
            ...
            ?array $customparams = null, $aud = '123'): lticourseshellMessageLaunch {

        $mocklaunch = $this->getMockBuilder(lticourseshellMessageLaunch::class)
        ...
        $mocklaunch->expects($this->any())
            ...
            ->will($this->returnCallback(
                ...
                use ($resource, $mockuser, $resourcelinkid, $migrationclaiminfo, $ags, $nrps, $customparams, $aud) {
                    ...
                    $rlticourseshelltle = $resourcelinkid ? "Resource link $resourcelinkid in platform" : "Resource link in platform";
                    ...
                    if (is_array($ags)) {
                        ...
                        $data["https://purl.imsglobal.org/spec/lticourseshell-ags/claim/endpoint"] = $agsclaim;
                    }

                    if ($nrps) {
                        $data['https://purl.imsglobal.org/spec/lticourseshell-nrps/claim/namesroleservice'] = [
        ...
        $mocklaunch->expects($this->any())
            ...
            ->will($this->returnCallback(function() {
                return uniqid('lticourseshell1p3_launch_', true);
            }));
    ...
    /**
     * Sets up and returns a test course, including lticourseshell-published resources, ready for testing.
     *
    ...
    /**
     * Enable enrol_lticourseshell plugin.
     */
    ...
    protected function setup_legacy_data(\stdClass $course, array $legacydata): array {
        ...
        $consumerrecord->id = $DB->insert_record('enrol_lti_cs_lticourseshell2_consumer', $consumerrecord);

        // Legacy data: create some modules and publish them as tools, using different secrets, over lticourseshell 1.1.
        $tools = [];
        ...
        $DB->insert_records('enrol_lti_cs_tool_consumer_map', $toolconsumermaprecords);

        // Legacy data: create the user who launched the tools over lticourseshell 1.1.
