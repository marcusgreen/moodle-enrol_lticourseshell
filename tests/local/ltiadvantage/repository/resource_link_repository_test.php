 ...
 * @coversDefaultClass \enrol_lticourseshell\local\ltiadvantage\repository\resource_link_repository
 */
final class resource_link_repository_test extends \advanced_testcase {
    ...
    protected function assert_resourcelink_db_values(resource_link $expected) {
        global $DB;
        $checkrecord = $DB->get_record('enrol_lti_cs_resource_link', ['id' => $expected->get_id()]);
        ...
        $this->assertEquals($expected->get_contextid(), $checkrecord->lticourseshellcontextid);
    ...
    public function test_delete(): void {
        ...
        $userrepo->save($user);
        global $DB;
        $this->assertTrue($DB->record_exists('enrol_lti_cs_user_resource_link',
        ...
        $this->assertFalse($DB->record_exists('enrol_lti_cs_user_resource_link',
            ['resourcelinkid' => $newreslink->get_id()]));
