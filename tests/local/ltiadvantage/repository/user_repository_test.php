 ...
 * @coversDefaultClass \enrol_lticourseshell\local\ltiadvantage\repository\user_repository
 */
final class user_repository_test extends \advanced_testcase {
    ...
    protected function generate_user(int $mockresourceid = 1, array $userfields = []): user {
        ...
        $lticourseshelluser = $savedresourcelink->add_user(
            $user->id,
            'source-id-123',
            ...array_values($userfields)
        );

        $lticourseshelluser->set_lastgrade(67.33333333);
    ...
    protected function assert_user_db_values(user $expected) {
        ...
        $lticourseshelluserrecord = $DB->get_record('enrol_lti_cs_users', ['id' => $expected->get_id()]);
        $this->assertEquals($expected->get_id(), $lticourseshelluserrecord->id);
        $this->assertEquals($expected->get_sourceid(), $lticourseshelluserrecord->sourceid);
        $this->assertEquals($expected->get_resourceid(), $lticourseshelluserrecord->toolid);
        $this->assertEquals($expected->get_lastgrade(), $lticourseshelluserrecord->lastgrade);

        if ($expected->get_resourcelinkid()) {
            $sql = "SELECT rl.id
                      FROM {enrol_lti_cs_users} lu
                      JOIN {enrol_lti_cs_user_resource_link} rlj
                        ON (lu.id = rlj.)
                      JOIN {enrol_lti_cs_resource_link} rl
    ...
    public function test_save_existing_localid_mismatch(): void {
        ...
        $this->expectExceptionMessage("Cannot update user mapping. lticourseshell user '{$saveduser->get_id()}' is already mapped " .
    ...
    public function test_save_stale_id(): void {
        ...
        $this->expectException(\coding_exception::class);
        $this->expectExceptionMessage("Cannot save lticourseshell user with id '999999'. The record does not exist.");
        $userrepo->save($user);
    }
    ...
    public function test_save_uniqueness_constraint(): void {
        ...
        $this->expectException(\coding_exception::class);
        $this->expectExceptionMessageMatches("/Cannot create duplicate lticourseshell user '[a-z0-9_]*' for resource '[0-9]*'/");
        $userrepo->save($user);
    }
    ...
    public function test_delete(): void {
        ...
        global $DB;
        $this->assertFalse($DB->record_exists('enrol_lti_cs_users', ['id' => $saveduser->get_id()]));
        $this->assertFalse($DB->record_exists('enrol_lti_cs_user_resource_link', ['lti_cs_userid' => $saveduser->get_id()]));
    ...
    /**
     * Test deleting a collection of lticourseshell user instances by deployment.
     *
    ...
    /**
     * Test confirming that any associated legacy lticourseshell user records are not returned by the repository.
     *
     * This test ensures that any enrolment methods (resources) updated in-place from legacy lticourseshell to 1.3 only return lticourseshell 1.3 users.
     *
     * @covers ::find
     * @covers ::find_single_user_by_resource
     * @covers ::find_by_resource
     */
    public function test_find_filters_legacy_lticourseshell_users(): void {
        $this->resetAfterTest();
        global $DB;
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course();
        $resource = $this->getDataGenerator()->create_lticourseshell_tool((object)['courseid' => $course->id]);
        $lticourseshelluserdata = [
            'userid' => $user->id,
            'toolid' => $resource->id,
            'sourceid' => '1001',
        ];
        $lti_cs_userid = $DB->insert_record('enrol_lti_cs_users', $lticourseshelluserdata);
        $userrepo = new user_repository();

        $this->assertNull($userrepo->find($lti_cs_userid));
        ...
        // Set deploymentid, indicating the user originated from an lticourseshell 1.3 launch and should now be returned.
        $lticourseshelluserdata['id'] = $lti_cs_userid;
        $lticourseshelluserdata['ltideploymentid'] = '234';
        $DB->update_record('enrol_lti_cs_users', $lticourseshelluserdata);

        $this->assertInstanceOf(user::class, $userrepo->find($lti_cs_userid));
        ...
        $this->assertInstanceOf(user::class, reset($lticourseshellusers));
    }
}
