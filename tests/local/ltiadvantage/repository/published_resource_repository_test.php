 ...
 * @coversDefaultClass \enrol_lticourseshell\local\ltiadvantage\repository\published_resource_repository
 */
final class published_resource_repository_test extends \advanced_testcase {
    ...
    protected function generate_published_resources() {
        ...
        $mod4 = $generator->create_module('workshop', ['course' => $course->id]); // Had multicourseshellple grade items.
        ...
        $coursetool = $generator->create_lticourseshell_tool($courseresourcedata);
        $coursetool = helper::get_lticourseshell_tool($coursetool->id);
        $modtool = $generator->create_lticourseshell_tool($moduleresourcedata);
        $modtool = helper::get_lticourseshell_tool($modtool->id);
        $mod2tool = $generator->create_lticourseshell_tool($module2resourcedata);
        $mod2tool = helper::get_lticourseshell_tool($mod2tool->id);
        $mod3tool = $generator->create_lticourseshell_tool($module3resourcedata);
        $mod3tool = helper::get_lticourseshell_tool($mod3tool->id);
        $mod4tool = $generator->create_lticourseshell_tool($module4resourcedata);
        $mod4tool = helper::get_lticourseshell_tool($mod4tool->id);
    ...
    public function test_find_all_for_user(): void {
        ...
        $this->assertFalse($resources[3]->supports_grades()); // Multicourseshellple grade items isn't supported in content selection.
    ...
    public function test_find_all_by_ids_for_user(): void {
        ...
        $this->assertFalse($resources[2]->supports_grades()); // Multicourseshellple grade items isn't supported in content selection.
    ...
    public function test_find_all_for_user_no_permissions(): void {
        ...
        // as if the user had launched into the module via lticourseshell enrolment, with an instructor role of 'editingteacher'.
