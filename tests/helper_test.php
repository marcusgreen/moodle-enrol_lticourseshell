...
final class helper_test extends \advanced_testcase {
    ...
    public function test_update_user_profile_image(): void {
        ...
        \enrol_lticourseshell\helper::update_user_profile_image($this->user1->id, $this->getExternalTestFileUrl('/test.jpg'));
    ...
    public function test_enrol_user_max_enrolled(): void {
        ...
        // Set up the lticourseshell enrolment tool.
        $data = new \stdClass();
        $data->maxenrolled = 1;
        $tool = $this->getDataGenerator()->create_lticourseshell_tool($data);

        // Now get all the information we need.
        $tool = \enrol_lticourseshell\helper::get_lticourseshell_tool($tool->id);

        // Enrol a user.
        $result = \enrol_lticourseshell\helper::enrol_user($tool, $this->user1->id);
        ...
        $result = \enrol_lticourseshell\helper::enrol_user($tool, $this->user2->id);

        // Check that this user was not enrolled and we are told why.
        $this->assertEquals(\enrol_lticourseshell\helper::ENROLMENT_MAX_ENROLLED, $result);
        $this->assertEquals(1, $DB->count_records('user_enrolments', array('enrolid' => $tool->enrolid)));
    }
    ...
    public function test_enrol_user_enrolment_not_started(): void {
        ...
        // Check that this user was not enrolled and we are told why.
        $this->assertEquals(\enrol_lticourseshell\helper::ENROLMENT_NOT_STARTED, $result);
    ...
    public function test_enrol_user_enrolment_finished(): void {
        ...
        // Check that this user was not enrolled and we are told why.
        $this->assertEquals(\enrol_lticourseshell\helper::ENROLMENT_FINISHED, $result);
        $this->assertEquals(0, $DB->count_records('user_enrolments', array('enrolid' => $tool->enrolid)));
    }
    ...
    public function test_count_lti_tools(): void {
        ...
        $generator->create_lticourseshell_tool($data);
        $generator->create_lticourseshell_tool($data);
        ...
        // Count all the tools.
        $count = \enrol_lticourseshell\helper::count_lti_tools();
        $this->assertEquals(4, $count);

        // Count all the tools in course 1.
        $count = \enrol_lticourseshell\helper::count_lti_tools(array('courseid' => $course1->id));
        $this->assertEquals(2, $count);

        // Count all the tools in course 2 that are disabled.
        $count = \enrol_lticourseshell\helper::count_lti_tools(array('courseid' => $course2->id, 'status' => ENROL_INSTANCE_DISABLED));
        $this->assertEquals(1, $count);

        // Count all the tools that are enabled.
        $count = \enrol_lticourseshell\helper::count_lti_tools(array('status' => ENROL_INSTANCE_ENABLED));
    ...
    public function test_get_lti_tools(): void {
        ...
        $data = new \stdClass();
        $data->courseid = $course1->id;
        $tool1 = $generator->create_lticourseshell_tool($data);
        $tool2 = $generator->create_lticourseshell_tool($data);

        // Create two more tools in a separate course.
        $course2 = $generator->create_course();
        $data = new \stdClass();
        $data->courseid = $course2->id;
        $tool3 = $generator->create_lticourseshell_tool($data);

        // Set the next tool to disabled.
        $data->status = ENROL_INSTANCE_DISABLED;
        $tool4 = $generator->create_lticourseshell_tool($data);

        // Get all the tools.
        $tools = \enrol_lticourseshell\helper::get_lti_tools();

        // Check that we got all the tools.
        $this->assertEquals(4, count($tools));

        // Get all the tools in course 1.
        $tools = \enrol_lticourseshell\helper::get_lti_tools(array('courseid' => $course1->id));
        ...
        $tools = \enrol_lticourseshell\helper::get_lti_tools(array('courseid' => $course2->id, 'status' => ENROL_INSTANCE_DISABLED));
        ...
        $tools = \enrol_lticourseshell\helper::get_lti_tools(array('status' => ENROL_INSTANCE_ENABLED));
    ...
    public function test_get_launch_url(): void {
        ...
        $data = new \stdClass();
        $data->courseid = $course1->id;
        $tool1 = $this->getDataGenerator()->create_lticourseshell_tool($data);

        $id = $tool1->id;
        $launchurl = \enrol_lticourseshell\helper::get_launch_url($id);
        $this->assertEquals('https://www.example.com/moodle/enrol/lticourseshell/tool.php?id=' . $id, $launchurl->out());
    }
    ...
    public function test_get_cartridge_url(): void {
        ...
        $id = $tool1->id;
        $token = \enrol_lticourseshell\helper::generate_cartridge_token($id);
        $launchurl = \enrol_lticourseshell\helper::get_cartridge_url($tool1);
        $this->assertEquals('https://www.example.com/moodle/enrol/lticourseshell/cartridge.php?id=' . $id . '&amp;token=' . $token,
                            $launchurl->out());

        $CFG->slasharguments = true;

        $launchurl = \enrol_lticourseshell\helper::get_cartridge_url($tool1);
        $this->assertEquals('https://www.example.com/moodle/enrol/lticourseshell/cartridge.php/' . $id . '/' . $token . '/cartridge.xml',
                            $launchurl->out());
    ...
    public function test_get_proxy_url(): void {
        ...
        $id = $tool1->id;
        $token = \enrol_lticourseshell\helper::generate_proxy_token($id);
        $launchurl = \enrol_lticourseshell\helper::get_proxy_url($tool1);
        $this->assertEquals('https://www.example.com/moodle/enrol/lticourseshell/proxy.php?id=' . $id . '&amp;token=' . $token,
                            $launchurl->out());

        $CFG->slasharguments = true;

        $launchurl = \enrol_lticourseshell\helper::get_proxy_url($tool1);
        $this->assertEquals('https://www.example.com/moodle/enrol/lticourseshell/proxy.php/' . $id . '/' . $token . '/',
                            $launchurl->out());
    ...
    public function test_get_name(): void {
        ...
        $name = \enrol_lticourseshell\helper::get_name($tool1);
    ...
    public function test_get_description(): void {
        ...
        $description = \enrol_lticourseshell\helper::get_description($tool1);
        ...
        $tool2 = $generator->create_lticourseshell_tool($data);
        $description = \enrol_lticourseshell\helper::get_description($tool2);
    ...
    public function test_get_icon(): void {
        ...
        $icon = \enrol_lticourseshell\helper::get_icon($tool);
        $icon = $icon->out();
        // Only local icons are supported by the lticourseshell framework.
    ...
    public function test_verify_cartridge_token(): void {
        ...
        $token = \enrol_lticourseshell\helper::generate_cartridge_token($tool1->id);
        $this->assertTrue(\enrol_lticourseshell\helper::verify_cartridge_token($tool1->id, $token));
        $this->assertFalse(\enrol_lticourseshell\helper::verify_cartridge_token($tool1->id, 'incorrect token!'));
    }
    ...
    public function test_verify_proxy_token(): void {
        ...
        $token = \enrol_lticourseshell\helper::generate_proxy_token($tool1->id);
        $this->assertTrue(\enrol_lticourseshell\helper::verify_proxy_token($tool1->id, $token));
        $this->assertFalse(\enrol_lticourseshell\helper::verify_proxy_token($tool1->id, 'incorrect token!'));
    }
    ...
    public function test_set_xpath($parameters, $expected): void {
        $helper = new \ReflectionClass('enrol_lticourseshell\\helper');
    ...
    public function test_create_cartridge(): void {
        ...
        $cartridge = \enrol_lticourseshell\helper::create_cartridge($tool1->id);
        $this->assertStringContainsString('<blticourseshell:title>Test lticourseshell</blticourseshell:title>', $cartridge);
        $this->assertStringContainsString("<blticourseshell:icon>$CFG->wwwroot/theme/image.php/boost/theme/1/favicon</blticourseshell:icon>", $cartridge);
        $this->assertStringContainsString("<blticourseshell:launch_url>$CFG->wwwroot/enrol/lticourseshell/tool.php?id=$tool1->id</blticourseshell:launch_url>", $cartridge);
    }
}
