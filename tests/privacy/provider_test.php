...
use enrol_lticourseshell\privacy\provider;
use stdClass;
...
final class provider_test extends \core_privacy\tests\provider_testcase {
    ...
    public function setUp(): void {
        ...
        // Create lticourseshell tools in different contexts.
        $this->create_lticourseshell_users($coursecontext, $this->user->id);
        $this->create_lticourseshell_users($coursecontext, $this->user->id);
        $this->create_lticourseshell_users($cmcontext, $this->user->id);

        // Create another lticourseshell user.
        $this->anotheruser = $this->getDataGenerator()->create_user();
        $this->create_lticourseshell_users($coursecontext, $this->anotheruser->id);
    }
    ...
    public function test_export_for_context(): void {
        ...
        $this->export_context_data_for_user($this->user->id, $coursecontext, 'enrol_lticourseshell');
        $writer = \core_privacy\local\request\writer::with_context($coursecontext);
        $this->assertTrue($writer->has_any_data());

        $data = (array) $writer->get_data(['enrol_lti_cs_users']);
        $this->assertCount(2, $data);
        foreach ($data as $lticourseshelluser) {
            $this->assertArrayHasKey('lastgrade', $lticourseshelluser);
            $this->assertArrayHasKey('timecreated', $lticourseshelluser);
            $this->assertArrayHasKey('timemodified', $lticourseshelluser);
        }

        // Export all of the data for the activity context.
        $this->export_context_data_for_user($this->user->id, $cmcontext, 'enrol_lticourseshell');
    ...
    public function test_delete_data_for_all_users_in_context(): void {
        ...
        $count = $DB->count_records('enrol_lti_cs_users');
        ...
        $lticourseshellusers = $DB->get_records('enrol_lti_cs_users');
        $this->assertCount(1, $lticourseshellusers);

        $lticourseshelluser = reset($lticourseshellusers);
        $this->assertEquals($lticourseshelluser->userid, $this->user->id);
    }
    ...
    public function test_delete_data_for_user(): void {
        ...
        $contextlist = new \core_privacy\local\request\approved_contextlist($this->user, 'enrol_lticourseshell',
        ...
        $lticourseshelluser = reset($lticourseshellusers);
        $this->assertNotEquals($lticourseshelluser->userid, $this->user->id);
    }

    /**
     * Creates a lticourseshell user given the provided context
     *
     * @param \context $context
     * @param int $userid
     */
    private function create_lticourseshell_users(\context $context, int $userid) {
        global $DB;

        // Create a tool.
        $lticourseshelltool = (object) [
            'enrolid' => 5,
        ...
        $toolid = $DB->insert_record('enrol_lti_cs_tools', $lticourseshelltool);

        // Create a user.
        $lticourseshelluser = (object) [
            'userid' => $userid,
            'toolid' => $toolid,
            'lastgrade' => 50,
        ...
        $DB->insert_record('enrol_lti_cs_users', $lticourseshelluser);
    }
    ...
    public function test_delete_data_for_users_course(): void {
        ...
        foreach ($lticourseshellusers as $lticourseshelluser) {
            $leftover = false;
            if ($lticourseshelluser->userid == $this->user->id) {
                $contextid = $DB->get_field('enrol_lti_cs_tools', 'contextid', ['id' => $lticourseshelluser->toolid]);
