...
final class tool_provider_test extends \advanced_testcase {
    ...
    /**
     * @var \stdClass $tool The lticourseshell tool.
     */
    protected $tool;
    ...
    public function setUp(): void {
        ...
        $data->secret = 'secret';
        $toolrecord = $this->getDataGenerator()->create_lticourseshell_tool($data);
        $this->tool = helper::get_lticourseshell_tool($toolrecord->id);
    ...
    public function test_on_error(): void {
        ...
        $tp->message = $message;
        $tp->onError();
        $errormessage = get_string('failedrequest', 'enrol_lticourseshell', ['reason' => $message]);
    ...
    public function test_on_register_no_consumer(): void {
        ...
        $this->assertFalse($tp->ok);
        $this->assertEquals(get_string('invalidtoolconsumer', 'enrol_lticourseshell'), $tp->message);
    }
    ...
    public function test_on_register_no_return_url(): void {
        ...
        $tp->onRegister();
        $this->assertFalse($tp->ok);
        $this->assertEquals(get_string('returnurlnotset', 'enrol_lticourseshell'), $tp->message);
    }
    ...
    public function test_on_register_failed(): void {
        ...
        $this->assertFalse($tp->ok);
        // Check message.
        $this->assertEquals(get_string('couldnotestablishproxy', 'enrol_lticourseshell'), $tp->message);
    }
    ...
    public function test_on_register(): void {
        ...
        $successmessage = get_string('successfulregistration', 'enrol_lticourseshell');
    ...
    public function test_on_launch_no_frame_embedding(): void {
        ...
        $this->assertStringContainsString(get_string('frameembeddingnotenabled', 'enrol_lticourseshell'), $output);
    }
    ...
    /**
     * Test for tool_provider::onLaunch() with invalid secret and no tool proxy (for lticourseshell 1 launches).
     */
    public function test_on_launch_with_invalid_secret_and_no_proxy(): void {
        ...
        @$tp->onLaunch();
        $this->assertFalse($tp->ok);
        $this->assertEquals(get_string('invalidrequest', 'enrol_lticourseshell'), $tp->message);
    }
            ...
            'tool_profile' => [
                'resource_handler' => [
                    [
                        'message' => [
                            [
                                'message_type' => 'basic-lticourseshell-launch-request',
                                'path' => '/enrol/lticourseshell/tool.php'
                            ]
                        ]
                    ]
                ]
                ...
                'resource_handler' => [
                    [
                        'message' => [
                            [
                                'message_type' => 'basic-lticourseshell-launch-request',
                                'path' => '/enrol/lticourseshell/tool.php?id=' . $tool->id
                            ]
                        ]
                    ]
                ]
    ...
    /**
     * Test for tool_provider::onLaunch() for a request with message type other than basic-lticourseshell-launch-request.
     */
    ...
    public function test_on_launch_with_user_image_from_resource_link(): void {
        ...
        $username = helper::create_username($tp->consumer->getKey(), $tp->user->lticourseshellUserId);
    ...
    /**
     * Test for tool_provider::onLaunch() to verify that a lticourseshell user has been enrolled.
     */
