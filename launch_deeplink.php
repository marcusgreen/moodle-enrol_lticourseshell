...
/**
 * Handles lticourseshell 1.3 deep linking launches.
 *
 ...
 */
 ...
 use enrol_lticourseshell\local\ltiadvantage\repository\published_resource_repository;
...
$cookie = new lticourseshell_cookie();
$serviceconnector = new ltiServiceConnector($sesscache, new http_client());
if ($idtoken) {
    $messagelaunch = lticourseshellMessageLaunch::new($issdb, $sesscache, $cookie, $serviceconnector)
        ->initialize($_POST);
}
if ($launchid) {
    $messagelaunch = lticourseshellMessageLaunch::fromCache($launchid, $issdb, $sesscache, $cookie, $serviceconnector);
}
...
$auth->complete_login(
    ...
    new moodle_url('/enrol/lticourseshell/launch_deeplink.php', ['launchid' => $messagelaunch->getLaunchId()]),
    auth_plugin_lticourseshell::PROVISIONING_MODE_PROMPT_EXISTING_ONLY
);

require_login(null, false);
global $USER, $CFG;
$PAGE->set_context(context_system::instance());
$url = new moodle_url('/enrol/lticourseshell/launch_deeplink.php');
$PAGE->set_url($url);
