...
/**
 * Defines the restore_enrol_lti_cs_plugin class.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_enrol_lti_cs_plugin extends restore_enrol_plugin {
    ...
    /**
     * Declares the enrol lticourseshell XML paths attached to the enrol element
     *
    ...
    protected function define_enrol_plugin_structure() {
        ...
        $paths = array();
        $paths[] = new restore_path_element('enrol_lti_cs_tool', $this->connectionpoint->get_path() . '/tool');
        $paths[] = new restore_path_element('enrol_lti_cs_users', $this->connectionpoint->get_path() . '/tool/users/user');

        return $paths;
    }

    /**
     * Processes lticourseshell tools element data
     *
     * @param array|stdClass $data
     */
    public function process_enrol_lti_cs_tool($data) {
        global $DB;
        ...
        $data->id = $DB->insert_record('enrol_lti_cs_tools', $data);

        // Add the array of tools we need to process later.
        $this->tools[$data->id] = $data;

        // Set up the mapping.
        $this->set_mapping('enrol_lti_cs_tool', $oldid, $data->id);
    }

    /**
     * Processes lticourseshell users element data
     *
     * @param array|stdClass $data The data to insert as a comment
     */
    public function process_enrol_lti_cs_users($data) {
        global $DB;

        $data = (object) $data;

        $data->userid = $this->get_mappingid('user', $data->userid);
        $data->toolid = $this->get_mappingid('enrol_lti_cs_tool', $data->toolid);
        $data->timecreated = time();

        $DB->insert_record('enrol_lti_cs_users', $data);
    }
    ...
    public function after_restore_enrol() {
        ...
        foreach ($this->tools as $tool) {
            ...
            $DB->update_record('enrol_lti_cs_tools', $updatetool);
        }
    }
}
