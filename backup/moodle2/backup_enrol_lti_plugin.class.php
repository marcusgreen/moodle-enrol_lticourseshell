...
/**
 * Defines the backup_enrol_lti_cs_plugin class.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_enrol_lti_cs_plugin extends backup_enrol_plugin {

    /**
     * Defines the other lticourseshell enrolment structures to append.
     *
    ...
    public function define_enrol_plugin_structure() {
        ...
        $tool->set_source_table('enrol_lti_cs_tools',
            array('enrolid' => backup::VAR_PARENTID));

        // Users are only added only if users included.
        if ($this->task->get_setting_value('users')) {
            $user->set_source_table('enrol_lti_cs_users', array('toolid' => backup::VAR_PARENTID));
        }
    }
}
