...
/**
 * lticourseshell 1.3 page to create or delete deployments.
 *
...
use core\output\notification;
use enrol_lticourseshell\local\ltiadvantage\form\deployment_form;
...
use enrol_lticourseshell\local\ltiadvantage\service\tool_deployment_service;
...
$deploymentslisturl = new moodle_url($CFG->wwwroot . "/enrol/lticourseshell/register_platform.php",
...
$pagesetup = function(string $pagetitle) {
    ...
    admin_externalpage_setup('enrolsettingslticourseshell_deployment_manage', '', null, '', ['pagelayout' => 'admin']);
...
if ($action === 'add') {
    $pagesetup(get_string('deploymentadd', 'enrol_lticourseshell'));

    $pageurl = new moodle_url('/enrol/lticourseshell/manage_deployment.php', ['action' => 'add']);
    ...
    if ($data = $mform->get_data()) {
        ...
        redirect($deploymentslisturl, get_string('deploymentaddnotice', 'enrol_lticourseshell'), null,
            notification::NOTIFY_SUCCESS);
    } else if (!$mform->is_cancelled()) {

        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('deploymentadd', 'enrol_lticourseshell'));
        $mform->set_data([
    ...
    $pagesetup(get_string('deploymentdelete', 'enrol_lticourseshell'));
    ...
    if (!optional_param('confirm', false, PARAM_BOOL)) {
        ...
        $continueurl = new moodle_url('/enrol/lticourseshell/manage_deployment.php', $continueparams);
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->confirm(
            get_string('deploymentdeleteconfirm', 'enrol_lticourseshell', format_string($deployment->get_deploymentid())),
            $continueurl,
            $deploymentslisturl
        );
        echo $OUTPUT->footer();
        ...
        redirect($deploymentslisturl,
            get_string('deploymentdeletenotice', 'enrol_lticourseshell'), null,  notification::NOTIFY_SUCCESS);
    }
}
