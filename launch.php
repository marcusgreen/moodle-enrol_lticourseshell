 ...
 *
 * See enrol/lticourseshell/launch_deeplink.php for deep linking launches.
 *
...
use enrol_lticourseshell\local\ltiadvantage\lib\lti_cookie;
...
use enrol_lticourseshell\local\ltiadvantage\repository\legacy_consumer_repository;
...
use enrol_lticourseshell\local\ltiadvantage\utility\message_helper;
...
// Auth code needs to be told about consumer secrets for the purposes of migration, since these reside in enrol_lticourseshell.
...
$resource = array_values(\enrol_lticourseshell\helper::get_lti_tools(['uuid' => $resourceuuid]));
$resource = $resource[0] ?? null;
if (empty($resource) || $resource->status != ENROL_INSTANCE_ENABLED) {
    throw new \moodle_exception('lticourseshelladvlauncherror:invalidid', 'enrol_lticourseshell', '', $resourceuuid);
}
...
$provisioningmode = message_helper::is_instructor_launch($launchdata) ? $resource->provisioningmodeinstructor
    ...
    $auth->complete_login(
        ...
        new moodle_url('/enrol/lticourseshell/launch.php', ['launchid' => $messagelaunch->getLaunchId()]),
        $provisioningmode,
...
$PAGE->set_url(new moodle_url('/enrol/lticourseshell/launch.php'), ['launchid' => $messagelaunch->getLaunchId()]);
