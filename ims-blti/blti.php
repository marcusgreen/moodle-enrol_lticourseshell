...
require_once($CFG->dirroot . '/enrol/lticourseshell/ims-blticourseshell/TrivialOAuthDataStore.php');

// Returns true if this is a Basic lticourseshell message
...
// Basic lticourseshell Class that does the setup and provides utility
// functions
class Blticourseshell {
    ...
    function __construct($parm=false, $usesession=true, $doredirect=true) {
        ...
        // If this request is not an lticourseshell Launch, either
        // give up or try to retrieve the context from session
        if ( ! is_basic_lticourseshell_request() ) {

            if ( $usesession === false ) return;

            if ( strlen(session_id()) > 0 ) {
                $row = $_SESSION['_basiclticourseshell_lticourseshell_row'];
                if ( isset($row) ) $this->row = $row;
                $context_id = $_SESSION['_basiclticourseshell_lticourseshell_context_id'];
                if ( isset($context_id) ) $this->context_id = $context_id;
                $info = $_SESSION['_basic_lticourseshell_context'];
                if ( isset($info) ) {
                    $this->info = $info;
                    $this->valid = true;
                    return;
        ...
        foreach($_POST as $key => $value ) {
            if ( $key == "basiclticourseshell_submit" ) continue;
        ...
        if (isset($newinfo['custom_lticourseshell_message_encoded_base64']) && $newinfo['custom_lticourseshell_message_encoded_base64']==1){
        ...
        if ( $usesession == true and strlen(session_id()) > 0 ) {
             $_SESSION['_basic_lticourseshell_context'] = $this->info;
             unset($_SESSION['_basiclticourseshell_lticourseshell_row']);
             unset($_SESSION['_basiclticourseshell_lticourseshell_context_id']);
             if ( $this->row ) $_SESSION['_basiclticourseshell_lticourseshell_row'] = $this->row;
             if ( $this->context_id ) $_SESSION['_basiclticourseshell_lticourseshell_context_id'] = $this->context_id;
        }
    ...
    function getUserEmail() {
        ...
        $email = $this->info['user_id'] . "@lticourseshelluser.com";
     ...
     */
     function decodeBase64($info) {
         $keysNoEncode = array("lticourseshell_version", "lticourseshell_message_type", "tool_consumer_instance_description", "tool_consumer_instance_guid", "oauth_consumer_key", "custom_lticourseshell_message_encoded_base64", "oauth_nonce", "oauth_version", "oauth_callback", "oauth_timestamp", "basiclticourseshell_submit", "oauth_signature_method", "ext_ims_lis_memberships_id", "ext_ims_lis_memberships_url");
