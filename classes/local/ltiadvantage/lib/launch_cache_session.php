...
namespace enrol_lticourseshell\local\ltiadvantage\lib;
...
class launch_cache_session implements ICache {
    ...
    public function getLaunchData(string $key): ?array {
        global $SESSION;
        if (isset($SESSION->enrol_lti_cs_launch[$key])) {
            return unserialize($SESSION->enrol_lti_cs_launch[$key]);
        }
        return null;
    }
    ...
    public function cacheLaunchData(string $key, array $jwtBody): void {
        global $SESSION;
        $SESSION->enrol_lti_cs_launch[$key] = serialize($jwtBody);
    }
    ...
    public function cacheNonce(string $nonce, string $state): void {
        global $SESSION;
        $SESSION->enrol_lti_cs_launch_nonce[$nonce] = $state;
    }
    ...
    public function checkNonceIsValid(string $nonce, string $state): bool {
        global $SESSION;
        return isset($SESSION->enrol_lti_cs_launch_nonce[$nonce]) && $SESSION->enrol_lti_cs_launch_nonce[$nonce] == $state;
    }
    ...
    public function purge() {
        global $SESSION;
        unset($SESSION->enrol_lti_cs_launch);
    }
    ...
    public function cacheAccessToken(string $key, string $accessToken): void {
        global $SESSION;
        $SESSION->enrol_lti_cs_launch_token[$key] = $accessToken;
    }
    ...
    public function getAccessToken(string $key): ?string {
        global $SESSION;
        return $SESSION->enrol_lti_cs_launch_token[$key] ?? null;
    }
    ...
    public function clearAccessToken(string $key): void {
        global $SESSION;
        unset($SESSION->enrol_lti_cs_launch_token[$key]);
    }
}
