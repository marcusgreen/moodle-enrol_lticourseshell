...
use Packback\Lti1p3\lticourseshellAssignmentsGradesService;
use Packback\Lti1p3\lticourseshellGrade;
use Packback\Lti1p3\lticourseshellLineitem;
...
class sync_tool_grades extends adhoc_task {
     ...
     *
     * @param \stdClass $resource the enrol_lti_cs_tools data record for the shared resource.
    ...
    protected function sync_grades_for_resource($resource): array {
        ...
        if ($users = $userrepo->find_by_resource($resource->id)) {
            ...
            foreach ($users as $user) {
                ...
                foreach ($userresourcelinks as $userresourcelink) {
                    ...
                    try {
                        ...
                        $lticourseshellgrade = lticourseshellGrade::new()
                            ->setScoreGiven($grade)
                        ...
                        if (empty($servicedata['lineitem'])) {
                            ...
                            $lineitem = $ags->findOrCreateLineitem(new lticourseshellLineitem([
                            ...
                            $response = $ags->putGrade($lticourseshellgrade, $lineitem);
                        } else {
                            // Let AGS find the coupled line item.
                            $response = $ags->putGrade($lticourseshellgrade);
                        }
            ...
            foreach ($syncedusergrades as $lticourseshelluser) {
                $userrepo->save($lticourseshelluser);
     ...
     *
     * @param \stdClass $resource the enrol_lti_cs_tools record.
     ...
     * @return lticourseshellAssignmentsGradesService
     */
    protected function get_ags(ltiServiceConnector $sc, LtiRegistration $registration, array $sd): lticourseshellAssignmentsGradesService {
        return new lticourseshellAssignmentsGradesService($sc, $registration, $sd);
    }
