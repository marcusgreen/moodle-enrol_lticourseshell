...
use enrol_lticourseshell\local\ltiadvantage\entity\nrps_info;
use enrol_lticourseshell\local\ltiadvantage\entity\resource_link;
use enrol_lticourseshell\local\ltiadvantage\entity\user;
use enrol_lticourseshell\local\ltiadvantage\lib\issuer_database;
use enrol_lticourseshell\local\ltiadvantage\lib\launch_cache_session;
...
use Packback\Lti1p3\lticourseshellNamesRolesProvisioningService;
...
class sync_members extends scheduled_task {
    ...
    /** @var user_repository $userrepo for fetching and saving lticourseshell user information.*/
    protected $userrepo;
    ...
    public function get_name(): string {
        return get_string('tasksyncmembers', 'enrol_lticourseshell');
    }
    ...
    protected function get_resource_link_level_members(nrps_info $nrps, ltiServiceConnector $sc, LtiRegistration $registration,
        ...
        $reslinklevelnrps = new lticourseshellNamesRolesProvisioningService($sc, $registration, $servicedata);
    ...
    protected function get_context_level_members(nrps_info $nrps, ltiServiceConnector $sc, LtiRegistration $registration) {
        ...
        $contextlevelnrps = new lticourseshellNamesRolesProvisioningService($sc, $registration, $clservicedata);
    ...
    public function execute() {
        ...
        if (!enrol_is_enabled('lticourseshell')) {
            mtrace('Skipping task - ' . get_string('enrolisdisabled', 'enrol_lticourseshell'));
            return;
        }
    ...
    protected function sync_unenrol_resourcelink(resource_link $resourcelink, stdClass $resource,
        ...
        foreach ($linkusers as $lticourseshelluser) {
            if (!in_array($lticourseshelluser->get_localid(), $syncedusers)) {
                $instance = new stdClass();
                $instance->id = $resource->enrolid;
                $instance->courseid = $resource->courseid;
                $instance->enrol = 'lticourseshell';
                $lticourseshellplugin->unenrol_user($instance, $lticourseshelluser->get_localid());
                $unenrolcount++;
            }
        }
        return $unenrolcount;
    ...
    /**
     * Creates an lticourseshell user object from a member entry.
     *
     ...
     * @return user the lticourseshell user instance.
     */
    protected function lticourseshelluser_from_member(stdClass $user, stdClass $resource,
            resource_link $resourcelink, array $member): user {

        if (!$lticourseshelluser = $this->userrepo->find_single_user_by_resource($user->id, $resource->id)) {
            // New user, so create them.
            $lticourseshelluser = user::create(
        ...
        $lticourseshelluser->set_lastaccess(time());
        return $lticourseshelluser;
    }
     ...
     * @param stdClass $resource the enrol_lti_cs_tools resource information.
     * @param resource_link $resourcelink the resource_link instance.
    ...
    protected function sync_member_information(application_registration $appregistration, stdClass $resource,
        ...
        // This will be used to locate legacy user accounts and link them to lticourseshell 1.3 users.
        ...
        foreach ($members as $member) {
            $auth = get_auth_plugin('lticourseshell');
            if ($auth->get_user_binding($appregistration->get_platformid()->out(false), $member['user_id'])) {
                ...
                if ($user->auth != 'lticourseshell') {
                    mtrace("Skipped profile sync for user '$user->id'. The user does not belong to the lticourseshell auth method.");
                }
            } else {
                ...
                switch ($provisioningmode) {
                    case \auth_plugin_lticourseshell::PROVISIONING_MODE_AUTO_ONLY:
                        // Automatic provisioning - this will create a user account and log the user in.
                        $user = $auth->find_or_create_user_from_membership($member, $appregistration->get_platformid()->out(false),
                            $legacyconsumerkey);
                        break;
                    case \auth_plugin_lticourseshell::PROVISIONING_MODE_PROMPT_NEW_EXISTING:
                    case \auth_plugin_lticourseshell::PROVISIONING_MODE_PROMPT_EXISTING_ONLY:
                    default:
            ...
            $lticourseshelluser = $this->lticourseshelluser_from_member($user, $resource, $resourcelink, $member);

            if ($this->should_sync_enrol($resource->membersyncmode)) {

                $lticourseshelluser->set_resourcelinkid($resourcelink->get_id());
                $lticourseshelluser = $this->userrepo->save($lticourseshelluser);
                if ($user->auth != 'lticourseshell') {
                    mtrace("Skipped picture sync for user '$user->id'. The user does not belong to the lticourseshell auth method.");
                } else {
                    if (isset($member['picture'])) {
                        $this->userphotos[$lticourseshelluser->get_localid()] = $member['picture'];
                    }
                }

                // Enrol the user in the course.
                if (helper::enrol_user($resource, $lticourseshelluser->get_localid()) === helper::ENROLMENT_SUCCESSFUL) {
                    $enrolcount++;
                }
            }
