...
class tool_deployment_service {
    ...
    public function delete_tool_deployment(int $deploymentid): void {
        ...
        global $DB, $CFG;
        $sql = "SELECT lu.userid as lti_cs_userid, e.*
                  FROM {enrol_lti_cs_users} lu
                  JOIN {enrol_lti_cs_tools} lt
                    ON (lt.id = lu.toolid)
                  JOIN {enrol} e
                    ON (e.id = lt.enrolid)
        ...
        $enrollticourseshell = new \enrol_lticourseshell_plugin();
        foreach ($instancesrs as $instance) {
            $userid = $instance->lti_cs_userid;
            $enrollticourseshell->unenrol_user($instance, $userid);
        }
        $instancesrs->close();

        // Delete any lticourseshell user enrolments.
