...
/**
 * The legacy_consumer_repository class, instances of which are responsible for querying lticourseshell 1.1/2.0 consumer info.
 *
...
class legacy_consumer_repository {
    ...
    /**
     * Get a list of all shared secrets which a given lticourseshell 1.1/2.0 consumer is associated with.
     *
    ...
    public function get_consumer_secrets(string $consumerkey): array {
        ...
        $sql = "SELECT t.id, t.secret
                  FROM {enrol_lti_cs_lticourseshell2_consumer} c
                  JOIN {enrol_lti_cs_tool_consumer_map} cm
                    ON (c.id = cm.consumerid)
                  JOIN {enrol_lti_cs_tools} t
                    ON (t.id = cm.toolid)
