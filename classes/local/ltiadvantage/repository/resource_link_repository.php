...
use enrol_lticourseshell\local\ltiadvantage\entity\deployment;
use enrol_lticourseshell\local\ltiadvantage\entity\resource_link;

/**
 * Class resource_link_repository.
 *
 * This class encapsulates persistence logic for \enrol_lticourseshell\local\entity\resource_link type objects.
 *
...
class resource_link_repository {
    ...
    private $table = 'enrol_lti_cs_resource_link';

    /** @var string the name of the table to which user-entity mappings will have been persisted. */
    private $userresourcelinkmaptable = 'enrol_lti_cs_user_resource_link';
     ...
     *
     * @param int $resourceid the local id of the resource (enrol_lti_cs_tools id)
     * @param int $userid the local id of the enrol_lticourseshell\local\ltiadvantage\user object
    ...
    public function find_by_resource_and_user(int $resourceid, int $userid): array {
        ...
        $sql = "SELECT r.id, r.resourcelinkid, r.resourceid, r.ltideploymentid, r.lticontextid, r.lineitemsservice,
                  ...
                  FROM {enrol_lti_cs_resource_link} r
                  JOIN {enrol_lti_cs_user_resource_link} ur
                    ON (r.id = ur.resourcelinkid)
                 WHERE ur.lti_cs_userid = :lti_cs_userid
                   AND r.resourceid = :resourceid";
        $records = $DB->get_records_sql($sql, ['lti_cs_userid' => $userid, 'resourceid' => $resourceid]);
    ...
    public function delete(int $id) {
        global $DB;
        // First remove all enrol_lti_cs_user_resource_link mappings.
