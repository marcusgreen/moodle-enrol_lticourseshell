...
namespace enrol_lticourseshell\local\ltiadvantage\repository;
use enrol_lticourseshell\local\ltiadvantage\entity\user;

/**
 * Class user_repository.
 *
 * This class encapsulates persistence logic for \enrol_lticourseshell\local\entity\user type objects.
 *
...
class user_repository {
    ...
    /** @var string $lticourseshelluserstable the name of the table to which the entity will be persisted.*/
    private $lticourseshelluserstable = 'enrol_lti_cs_users';

    /** @var string $userresourcelinkidtable the name of the join table mapping users to resource links.*/
    private $userresourcelinkidtable = 'enrol_lti_cs_user_resource_link';
    ...
    /**
     * Create the corresponding enrol_lti_cs_user record from a user instance.
     *
     * @param user $user the user instance.
     * @return \stdClass the record.
     */
    private function lticourseshell_user_record_from_user(user $user): \stdClass {
        $record = [
     ...
     *
     * The DB cannot be relied on to do this uniqueness check, since the table is shared by lticourseshell 1.1/2.0 data.
     *
     * @param user $user the user instance.
    ...
    private function user_exists_for_tool(user $user): bool {
        ...
        // lticourseshell_user entry (where tool matches the user object's resource).
        global $DB;
        $uniquesql = "SELECT lu.id
                        FROM {{$this->lticourseshelluserstable}} lu
    ...
    public function save(user $user): user {
        ...
        if ($id && !$exists) {
            throw new \coding_exception("Cannot save lticourseshell user with id '{$id}'. The record does not exist.");
        }

        $userrecord = $this->user_record_from_user($user);
        $lticourseshelluserrecord = $this->lticourseshell_user_record_from_user($user);
        $timenow = time();
        global $CFG;
        require_once($CFG->dirroot . '/user/lib.php');
        if ($exists) {
            $lticourseshelluser = $DB->get_record($this->lticourseshelluserstable, ['id' => $lticourseshelluserrecord->id]);
            $userid = $lticourseshelluser->userid;
            // Warn about localid vs lticourseshelluser->userid mismatches here. Callers shouldn't be able to force updates using
            // localid. Only new user associations can be created that way.
            if (!empty($userrecord->id) && $userid != $userrecord->id) {
                throw new \coding_exception("Cannot update user mapping. lticourseshell user '{$lticourseshelluser->id}' is already mapped " .
                    "to user '{$lticourseshelluser->userid}' and can't be associated with another user '{$userrecord->id}'.");
            }
            ...
            $lticourseshelluserrecord->timemodified = $timenow;
            $DB->update_record($this->lticourseshelluserstable, $lticourseshelluserrecord);
        } else {
            // Validate uniqueness of the lticourseshell user, in the case of a stale object coming in to be saved.
            if ($this->user_exists_for_tool($user)) {
                throw new \coding_exception("Cannot create duplicate lticourseshell user '{$user->get_localid()}' for resource " .
                    "'{$user->get_resourceid()}'.");
            }
            ...
            // Create the lticourseshell_user record, holding details that have a lifespan equal to that of the enrolment instance.
            $lticourseshelluserrecord->timecreated = $lticourseshelluserrecord->timemodified = $timenow;
            $lticourseshelluserrecord->userid = $userid;
            $lticourseshelluserrecord->id = $DB->insert_record($this->lticourseshelluserstable, $lticourseshelluserrecord);
        }

        // If the user was created via a resource_link, create that association.
        if ($reslinkid = $user->get_resourcelinkid()) {
            $resourcelinkmap = ['lti_cs_userid' => $lticourseshelluserrecord->id, 'resourcelinkid' => $reslinkid];
        ...
        $record = (object) array_merge(
            (array) $userrecord,
            (array) $lticourseshelluserrecord,
            $resourcelinkmap,
            ['localid' => $userid]
        );
    ...
    /**
     * Find an lticourseshell user instance by resource.
     *
     * @param int $userid the id of the moodle user to look for.
     * @param int $resourceid the id of the published resource.
     * @return user|null the lticourseshell user instance, or null if not found.
     */
    ...
    public function exists(int $id): bool {
        global $DB;
        return $DB->record_exists($this->lticourseshelluserstable, ['id' => $id]);
    }
    ...
    public function delete(int $id) {
        global $DB;
        $DB->delete_records($this->lticourseshelluserstable, ['id' => $id]);
        $DB->delete_records($this->userresourcelinkidtable, ['lti_cs_userid' => $id]);
    }

    /**
     * Delete all lticourseshell user instances based on a given local deployment instance id.
     *
    ...
    public function delete_by_deployment(int $deploymentid): void {
        global $DB;
        $DB->delete_records($this->lticourseshelluserstable, ['ltideploymentid' => $deploymentid]);
    }
}
