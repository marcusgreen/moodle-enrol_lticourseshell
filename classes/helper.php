...
/**
 * lticourseshell enrolment plugin helper.
 *
...
/**
 * lticourseshell enrolment plugin helper class.
 *
 * @package enrol_lticourseshell
 * @copyright 2016 Mark Nelson <markn@moodle.com>
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class helper {
     ...
     * @param string $lti_cs_userid External tool user id
     * @return string The new username
     */
    public static function create_username($consumerkey, $lti_cs_userid) {
        if (!empty($lti_cs_userid) && !empty($consumerkey)) {
            $userkey = $consumerkey . ':' . $lti_cs_userid;
        } else {
            $userkey = false;
        }

        return 'enrol_lti_cs_userid' . sha1($consumerkey . '::' . $userkey);
    }
     ...
     *
     * @param \stdclass $tool The tool object (retrieved using self::get_lticourseshell_tool() or self::get_lti_tools())
    ...
    public static function enrol_user($tool, $userid) {
        ...
        if (!$DB->record_exists('user_enrolments', array('enrolid' => $tool->enrolid, 'userid' => $userid))) {
            ...
            $lticourseshellenrol = enrol_get_plugin('lticourseshell');

            // Hack - need to do this to workaround DB caching hack. See MDL-53977.
            $timestart = intval(substr(time(), 0, 8) . '00') - 1;
            $lticourseshellenrol->enrol_user($instance, $userid, null, $timestart, $timeend);
        }
    ...
    /**
     * Returns the lticourseshell tool.
     *
     * @param int $toolid
    ...
    /**
     * Returns the lticourseshell tools requested.
     *
    ...
    /**
     * Returns the number of lticourseshell tools.
     *
     * @param array $params The list of SQL params (eg. array('columnname' => value, 'columnname2' => value)).
    ...
    public static function create_service_body($source, $grade) {
        return '<?xml version="1.0" encoding="UTF-8"?>
            <imsx_POXEnvelopeRequest xmlns="http://www.imsglobal.org/services/lticourseshellv1p1/xsd/imsoms_v1p0">
              <imsx_POXHeader>
    ...
    /**
     * Returns the url to launch the lticourseshell tool.
     *
    ...
    public static function get_launch_url($toolid) {
        return new \moodle_url('/enrol/lticourseshell/tool.php', array('id' => $toolid));
    }

    /**
     * Returns the name of the lticourseshell enrolment instance, or the name of the course/module being shared.
     *
     * @param \stdClass $tool The lticourseshell tool
    ...
    /**
     * Returns a description of the course or module that this lticourseshell instance points to.
     *
     * @param \stdClass $tool The lticourseshell tool
    ...
    public static function get_cartridge_url($tool) {
        ...
        if ($CFG->slasharguments) {
            $url = new \moodle_url('/enrol/lticourseshell/cartridge.php/' . $id . '/' . $token . '/cartridge.xml');
        } else {
            $url = new \moodle_url('/enrol/lticourseshell/cartridge.php',
                    array(
                        'id' => $id,
                        'token' => $token
                    )
    ...
    public static function get_proxy_url($tool) {
        ...
        if ($CFG->slasharguments) {
            $url = new \moodle_url('/enrol/lticourseshell/proxy.php/' . $id . '/' . $token . '/');
        } else {
            $url = new \moodle_url('/enrol/lticourseshell/proxy.php',
                    array(
                        'id' => $id,
                        'token' => $token
                    )
    ...
    public static function generate_cartridge_token($toolid) {
        ...
        $checkhash = md5($siteidentifier . '_enrol_lti_cs_cartridge_' . $toolid);
        return $checkhash;
    }
    ...
    public static function generate_proxy_token($toolid) {
        $siteidentifier = get_site_identifier();
        $checkhash = md5($siteidentifier . '_enrol_lti_cs_proxy_' . $toolid);
        return $checkhash;
    }
    ...
    protected static function get_cartridge_parameters($toolid) {
        ...
        // Get the tool.
        $tool = self::get_lticourseshell_tool($toolid);
        ...
        return array(
                "/cc:cartridge_basiclticourseshell_link" => array(
                    "/blticourseshell:title" => $title,
                    "/blticourseshell:description" => $description,
                    "/blticourseshell:extensions" => array(
                            "/lticourseshellcm:property[@name='icon_url']" => $iconurl,
                            "/lticourseshellcm:property[@name='secure_icon_url']" => $secureiconurl
                        ),
                    "/blticourseshell:launch_url" => $launchurl,
                    "/blticourseshell:secure_launch_url" => $securelaunchurl,
                    "/blticourseshell:icon" => $iconurl,
                    "/blticourseshell:secure_icon" => $secureiconurl,
                    "/blticourseshell:vendor" => array(
                            "/lticourseshellcp:code" => $SITE->shortname,
                            "/lticourseshellcp:name" => $SITE->fullname,
                            "/lticourseshellcp:description" => trim(html_to_text($SITE->summary)),
                            "/lticourseshellcp:url" => $vendorurl
                        )
                )
            );
    }
    ...
    public static function create_cartridge($toolid) {
        ...
        $cartridge->load(realpath(__DIR__ . '/../xml/imslticourseshellcc.xml'));
        $xpath = new \DOMXpath($cartridge);
        $xpath->registerNamespace('cc', 'http://www.imsglobal.org/xsd/imslticourseshellcc_v1p0');
