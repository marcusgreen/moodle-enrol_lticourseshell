...
/**
 * Handles synchronising grades for the enrolment lticourseshell.
 *
...
namespace enrol_lticourseshell\task;

/**
 * Task for synchronising grades for the enrolment lticourseshell.
 *
 * @package    enrol_lticourseshell
 * @copyright  2016 Mark Nelson <markn@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class sync_grades extends \core\task\scheduled_task {
    ...
    public function get_name() {
        return get_string('tasksyncgrades', 'enrol_lticourseshell');
    }
    ...
    public function execute() {
        ...
        require_once($CFG->dirroot . '/enrol/lticourseshell/ims-blticourseshell/OAuth.php');
        require_once($CFG->dirroot . '/enrol/lticourseshell/ims-blticourseshell/OAuthBody.php');
        ...
        if ($tools = \enrol_lticourseshell\helper::get_lti_tools(array('status' => ENROL_INSTANCE_ENABLED, 'gradesync' => 1,
            ...
            foreach ($tools as $tool) {
                ...
                if ($lticourseshellusers = $DB->get_records('enrol_lti_cs_users', array('toolid' => $tool->id), 'lastaccess DESC')) {
                    $completion = new \completion_info(get_course($tool->courseid));
                    foreach ($lticourseshellusers as $lticourseshelluser) {
                        $mtracecontent = "for the user '$lticourseshelluser->userid' in the tool '$tool->id' for the course " .
                            "'$tool->courseid'";

                        $usercount = $usercount + 1;

                        // Check if we do not have a serviceurl - this can happen if the sync process has an unexpected error.
                        if (empty($lticourseshelluser->serviceurl)) {
                        ...
                        if (empty($lticourseshelluser->sourceid)) {
                        ...
                        if ($context->contextlevel == CONTEXT_COURSE) {
                            ...
                            if ($tool->gradesynccompletion && !$completion->is_course_complete($lticourseshelluser->userid)) {
                                mtrace("Skipping - Course not completed $mtracecontent.");
                                continue;
                            }

                            // Get the grade.
                            if ($grade = grade_get_course_grade($lticourseshelluser->userid, $tool->courseid)) {
                            ...
                            if ($tool->gradesynccompletion) {
                                $data = $completion->get_data($cm, false, $lticourseshelluser->userid);
                            ...
                            $grades = grade_get_grades($cm->course, 'mod', $cm->modname, $cm->instance, $lticourseshelluser->userid);
                        ...
                        if (!grade_floats_different($grade, $lticourseshelluser->lastgrade)) {
                        ...
                        $body = \enrol_lticourseshell\helper::create_service_body($lticourseshelluser->sourceid, $floatgrade);

                        try {
                            $response = sendOAuthBodyPOST('POST', $lticourseshelluser->serviceurl,
                                $lticourseshelluser->consumerkey, $lticourseshelluser->consumersecret, 'application/xml', $body);
                        } catch (\Exception $e) {
                        ...
                        if (strpos(strtolower($response), 'success') !== false) {
                            $DB->set_field('enrol_lti_cs_users', 'lastgrade', grade_floatval($grade), array('id' => $lticourseshelluser->id));
