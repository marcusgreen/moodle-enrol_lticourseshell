...
/**
 * Handles synchronising members using the enrolment lticourseshell.
 *
...
/**
 * Task for synchronising members using the enrolment lticourseshell.
 *
 * @package    enrol_lticourseshell
 * @copyright  2016 Mark Nelson <markn@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class sync_members extends scheduled_task {
    ...
    protected function sync_member_information(stdClass $tool, ToolConsumer $consumer, $members) {
        ...
        foreach ($members as $member) {
            // Set the user data.
            $user = new stdClass();
            $user->username = helper::create_username($consumer->getKey(), $member->lticourseshellUserId);
            ...
            if ($this->should_sync_enrol($tool->membersyncmode)) {
                ...
                // Check if this user has already been registered in the enrol_lti_cs_users table.
                if (!$DB->record_exists('enrol_lti_cs_users', ['toolid' => $tool->id, 'userid' => $user->id])) {
                    // Create an initial enrol_lti_cs_user record that we can use later when syncing grades and members.
    ...
    protected function sync_unenrol(stdClass $tool, string $consumerkey, array $currentusers) {
        ...
        $lticourseshellusersrs = $DB->get_recordset_select('enrol_lti_cs_users', $select, ['toolid' => $tool->id, 'consumerkey' => $consumerkey],
            'lastaccess DESC', 'userid');
        // Go through the users and check if any were never listed, if so, remove them.
        foreach ($lticourseshellusersrs as $lticourseshelluser) {
            if (!array_key_exists($lticourseshelluser->userid, $currentusers)) {
                $instance = new stdClass();
                $instance->id = $tool->enrolid;
                $instance->courseid = $tool->courseid;
                $instance->enrol = 'lticourseshell';
                $lticourseshellplugin->unenrol_user($instance, $lticourseshelluser->userid);
                // Increment unenrol count.
                $unenrolcount++;
            }
        }
        $lticourseshellusersrs->close();
    ...
    /**
     * Performs membership service request using an lticourseshell Context object.
     *
