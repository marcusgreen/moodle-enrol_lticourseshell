...
/**
 * Extends the IMS Tool provider library for the lticourseshell enrolment.
 *
...
use core\notification;
use core_user;
use enrol_lticourseshell\output\registration;
use html_writer;
...
class tool_provider extends ToolProvider {
    ...
    /**
     * @var stdClass $tool The object representing the enrol instance providing this lticourseshell tool
     */
    protected $tool;
    ...
    /**
     * Create a new instance of tool_provider to handle all the lticourseshell tool provider interactions.
     *
    ...
    protected function onError() {
        ...
        if (empty($this->returnUrl)) {
            $this->errorOutput = $OUTPUT->notification(get_string('failedrequest', 'enrol_lticourseshell', ['reason' => $message]), 'error');
        }
    }
    ...
    protected function onLaunch() {
        ...
        if (empty($this->consumer) || $this->dataConnector->loadToolConsumer($this->consumer) === false) {
            $this->ok = false;
            $this->message = get_string('invalidtoolconsumer', 'enrol_lticourseshell');
            return;
        }
        ...
        if (!$correctlaunchurl) {
            $this->ok = false;
            $this->message = get_string('invalidrequest', 'enrol_lticourseshell');
            return;
        }
        ...
        // Set the user data.
        $user = new stdClass();
        $user->username = helper::create_username($this->consumer->getKey(), $this->user->lticourseshellUserId);
        ...
        if ($result !== helper::ENROLMENT_SUCCESSFUL) {
            throw new \moodle_exception($result, 'enrol_lticourseshell');
            exit();
        }
        ...
        // Login user.
        $sourceid = $this->user->lticourseshellResultSourcedId;
        $serviceurl = $this->resourceLink->getSetting('lis_outcome_service_url');

        // Check if we have recorded this user before.
        if ($userlog = $DB->get_record('enrol_lti_cs_users', ['toolid' => $tool->id, 'userid' => $user->id])) {
            ...
            $DB->update_record('enrol_lti_cs_users', $userlog);
        } else {
            ...
            $DB->insert_record('enrol_lti_cs_users', $userlog);
        }
        ...
        if (empty($CFG->allowframembedding)) {
            ...
            $stropentool = get_string('opentool', 'enrol_lticourseshell');
            echo html_writer::tag('p', get_string('frameembeddingnotenabled', 'enrol_lticourseshell'));
    ...
    protected function onRegister() {
        ...
        if (empty($this->returnUrl)) {
            $this->ok = false;
            $this->message = get_string('returnurlnotset', 'enrol_lticourseshell');
            return;
        }
        ...
        if ($this->doToolProxyService()) {
            ...
            $this->message = get_string('successfulregistration', 'enrol_lticourseshell');

            // Prepare response.
            $returnurl = new moodle_url($this->returnUrl);
            $returnurl->param('lticourseshell_msg', get_string("successfulregistration", "enrol_lticourseshell"));
            ...
            $output = $PAGE->get_renderer('enrol_lticourseshell');
            echo $output->render($registration);

        } else {
            // Tell the consumer that the registration failed.
            $this->ok = false;
            $this->message = get_string('couldnotestablishproxy', 'enrol_lticourseshell');
        }
    }
    ...
    public function map_tool_to_consumer() {
        ...
        if (empty($this->consumer)) {
            throw new moodle_exception('invalidtoolconsumer', 'enrol_lticourseshell');
        }
        ...
        $mappingexists = $DB->record_exists('enrol_lti_cs_tool_consumer_map', $mappingparams);
        if (!$mappingexists) {
            $DB->insert_record('enrol_lti_cs_tool_consumer_map', (object) $mappingparams);
        }
    }
}
