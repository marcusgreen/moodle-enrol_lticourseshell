...
class data_connector extends DataConnector {
    ...
    public function __construct() {
        parent::__construct(null, 'enrol_lti_cs_');
    ...
    public function getUserResultSourcedIDsResourceLink($resourcelink, $localonly, $idscope) {
        ...
        $fields = 'id, lticourseshellresultsourcedid, lticourseshelluserkey, created, updated';

        // Fetch records.
        $rs = $DB->get_recordset_select($this->userresulttable, $where, $params, '', $fields);
        foreach ($rs as $row) {
            $user = User::fromResourceLink($resourcelink, $row->lticourseshelluserkey);
            $user->setRecordId($row->id);
            $user->lticourseshellResultSourcedId = $row->lticourseshellresultsourcedid;
    ...
    public function loadUser($user) {
        ...
        $fields = 'id, resourcelinkid, lticourseshelluserkey, lticourseshellresultsourcedid, created, updated';
        if (!empty($userid)) {
            ...
            $row = $DB->get_record_select(
                ...
                "resourcelinkid = ? AND lticourseshelluserkey = ?",
        ...
        if ($row) {
            ...
            $user->lticourseshellUserId = $row->lticourseshelluserkey;
            $user->lticourseshellResultSourcedId = $row->lticourseshellresultsourcedid;
            $user->created = $row->created;
            $user->updated = $row->updated;
            return true;
    ...
    public function saveUser($user) {
        ...
        $params = [
            'lticourseshellresultsourcedid' => $user->lticourseshellResultSourcedId,
            'updated' => $user->updated
        ];

        if ($isinsert) {
            $params['resourcelinkid'] = $user->getResourceLink()->getRecordId();
            $params['lticourseshelluserkey'] = $user->getId(ToolProvider\ToolProvider::ID_SCOPE_ID_ONLY);
            $user->created = $now;
    ...
    public function get_contexts_from_consumer(ToolConsumer $consumer) {
        ...
        $contexts = [];
        $contextrecords = $DB->get_records($this->contexttable, ['consumerid' => $consumer->getRecordId()], '', 'lticourseshellcontextkey');
        foreach ($contextrecords as $record) {
            $context = Context::fromConsumer($consumer, $record->lticourseshellcontextkey);
            $contexts[] = $context;
        }
    ...
    public function get_consumers_mapped_to_tool($toolid) {
        ...
        $consumers = [];
        $consumerrecords = $DB->get_records('enrol_lti_cs_tool_consumer_map', ['toolid' => $toolid], '', 'consumerid');
