...
/**
 * Renderer class for lticourseshell enrolment
 *
...
namespace enrol_lticourseshell\output;
...
class renderer extends plugin_renderer_base {
    /**
     * Render the enrol_lticourseshell/proxy_registration template
     *
    ...
    public function render_registration(registration $registration) {
        ...
        return parent::render_from_template("enrol_lticourseshell/proxy_registration", $data);
    }
     ...
     * a list of lticourseshell Resource Link Content Items being sent back to the platform, allowing resource link creation to
     * take place.
     *
     * @param lticourseshellMessageLaunch $launch the launch data.
     * @param array $resources array of published resources available to the current user.
     * @return string html
     */
    public function render_published_resource_selection_view(lticourseshellMessageLaunch $launch, array $resources): string {
        global $CFG;
        $context = [
            'action' => $CFG->wwwroot . '/enrol/lticourseshell/configure.php',
        ...
        return parent::render_from_template('enrol_lticourseshell/local/ltiadvantage/content_select', $context);
    }
    ...
    public function render_admin_setting_registered_platforms(array $registrations): string {
        $registrationscontext = [
            'registrations' => [],
            'addurl' => (new \moodle_url('/enrol/lticourseshell/register_platform.php', ['action' => 'add']))->out(false),
        ];
        ...
        foreach ($registrations as $reg) {
            ...
            $status = get_string('registrationstatuspending', 'enrol_lticourseshell');
            if ($reg->is_complete()) {
                $status = get_string('registrationstatusactive', 'enrol_lticourseshell');
            }
            $registrationscontext['registrations'][] = [
                ...
                'statusstring' => $status,
                'tooldetailsurl' => (new \moodle_url('/enrol/lticourseshell/register_platform.php',
                    ['action' => 'view', 'regid' => $reg->get_id(), 'tabselect' => 'tooldetails']))->out(false),
                'platformdetailsurl' => (new \moodle_url('/enrol/lticourseshell/register_platform.php',
                    ['action' => 'view', 'regid' => $reg->get_id(), 'tabselect' => 'platformdetails']))->out(false),
                'deploymentsurl' => (new \moodle_url('/enrol/lticourseshell/register_platform.php',
                    ['action' => 'view', 'regid' => $reg->get_id(), 'tabselect' => 'tooldeployments']))->out(false),
                'deleteurl' => (new \moodle_url('/enrol/lticourseshell/register_platform.php',
        ...
        $versionnotice = new notification(
            get_string('registeredplatformsltiversionnotice', 'enrol_lticourseshell'),
            notification::NOTIFY_INFO
        );
        $versionnotice->set_show_closebutton(false);

        $return = parent::render($versionnotice);
        $return .= parent::render_from_template('enrol_lticourseshell/local/ltiadvantage/registered_platforms',
            $registrationscontext);
        return $return;
    }
     ...
     * Valid values correspond to the tab names in the enrol_lticourseshell/local/ltiadvantage/registration_view template, currently:
    ...
    public function render_registration_view(int $registrationid, string $activetab = '') {
        ...
        foreach ($deployments as $deployment) {
            $deploymentscontext[] = [
                ...
                'deleteurl' => (new \moodle_url(
                    '/enrol/lticourseshell/manage_deployment.php',
                    ['action' => 'delete', 'id' => $deployment->get_id(), 'registrationid' => $registration->get_id()]
                ))->out(false)
            ];
        }

        $regurl = new \moodle_url('/enrol/lticourseshell/register.php', ['token' => $registration->get_uniqueid()]);
        ...
        $tcontext = [
            ...
            'back_url' => (new \moodle_url('/admin/settings.php', ['section' => 'enrolsettingslticourseshell_registrations']))->out(false),
            'dynamic_registration_info' => get_string(
                'registrationurlinfomessage',
                'enrol_lticourseshell',
                get_docs_url('Publish_as_lticourseshell_tool')
            ),
            'dynamic_registration_url' => [
                'name' => get_string('registrationurl', 'enrol_lticourseshell'),
                'url' => $regurl,
                'id' => uniqid()
            ],
            'manual_registration_info' => get_string('endpointltiversionnotice', 'enrol_lticourseshell'),
            'manual_registration_urls' => [
                [
                    'name' => get_string('toolurl', 'enrol_lticourseshell'),
                    'url' => $CFG->wwwroot . '/enrol/lticourseshell/launch.php',
                    'id' => uniqid()
                ],
                [
                    'name' => get_string('loginurl', 'enrol_lticourseshell'),
                    'url' => $CFG->wwwroot . '/enrol/lticourseshell/login.php?id=' . $registration->get_uniqueid(),
                    'id' => uniqid()
                ],
                [
                    'name' => get_string('jwksurl', 'enrol_lticourseshell'),
                    'url' => $CFG->wwwroot . '/enrol/lticourseshell/jwks.php',
                    'id' => uniqid()
                ],
                [
                    'name' => get_string('deeplinkingurl', 'enrol_lticourseshell'),
                    'url' => $CFG->wwwroot . '/enrol/lticourseshell/launch_deeplink.php',
                    'id' => uniqid()
                ],
            ],
            'platform_details_info' => get_string('platformdetailsinfo', 'enrol_lticourseshell'),
            'platform_details' => [
                [
                    'name' => get_string('registerplatform:name', 'enrol_lticourseshell'),
                    'value' => $registration->get_name()
                ],
                [
                    'name' => get_string('registerplatform:platformid', 'enrol_lticourseshell'),
                    'value' => $registration->get_platformid() ?? '',
                ],
                [
                    'name' => get_string('registerplatform:clientid', 'enrol_lticourseshell'),
                    'value' => $registration->get_clientid() ?? '',
                ],
                [
                    'name' => get_string('registerplatform:authrequesturl', 'enrol_lticourseshell'),
                    'value' => $registration->get_authenticationrequesturl() ?? '',
                ],
                [
                    'name' => get_string('registerplatform:jwksurl', 'enrol_lticourseshell'),
                    'value' => $registration->get_jwksurl() ?? '',
                ],
                [
                    'name' => get_string('registerplatform:accesstokenurl', 'enrol_lticourseshell'),
                    'value' => $registration->get_accesstokenurl() ?? '',
                ]
            ],
            'edit_platform_details_url' => (new \moodle_url('/enrol/lticourseshell/register_platform.php',
                ['action' => 'edit', 'regid' => $registration->get_id()]))->out(false),
            'deployments_info' => get_string('deploymentsinfo', 'enrol_lticourseshell'),
            'has_deployments' => !empty($deploymentscontext),
            'tool_deployments' => $deploymentscontext,
            'add_deployment_url' => (new \moodle_url('/enrol/lticourseshell/manage_deployment.php',
                ['action' => 'add', 'registrationid' => $registrationid]))->out(false)
        ];

        return parent::render_from_template('enrol_lticourseshell/local/ltiadvantage/registration_view',
            $tcontext);
    }
    ...
    public function render_cookies_required_notice(): string {
        $notification = new notification(get_string('cookiesarerequiredinfo', 'enrol_lticourseshell'), notification::NOTIFY_WARNING, false);
        $tcontext = [
            'heading' => get_string('cookiesarerequired', 'enrol_lticourseshell'),
            'notification' => $notification->export_for_template($this),
        ];

        return parent::render_from_template('enrol_lticourseshell/local/ltiadvantage/cookies_required_notice', $tcontext);
    }
}
