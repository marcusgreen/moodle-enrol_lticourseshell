...
use enrol_lticourseshell\local\ltiadvantage\lib\lticourseshell_cookie;
...
foreach ($resources as $resource) {
    ...
    $contentitem = Resource::new()
        ->setUrl($CFG->wwwroot . '/enrol/lticourseshell/launch.php')
    ...
    if ($resource->supports_grades() && in_array($resource->get_id(), $grades)) {
        ...
        $lineitem = lticourseshellLineitem::new()
...
$url = new moodle_url('/enrol/lticourseshell/configure.php');
$PAGE->set_url($url);
$PAGE->set_pagelayout('popup');
echo $OUTPUT->header();
$dl = $messagelaunch->getDeepLink();

$formactionurl = $messagelaunch->getLaunchData()[lticourseshellConstants::DL_DEEP_LINK_SETTINGS]['deep_link_return_url'];
echo <<<HTML
